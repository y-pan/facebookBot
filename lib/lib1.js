...
module.exports.retrieveTeById = (ids, callback) => {
    // assume ids passed in is always single for Te 
    let text;
    for(let i=0; i< db.te.length; i++){
        if(db.te[i].id == ids[0]) {text=db.te[i].data; break;}
    }
...
module.exports.retrieveBuById = (ids, callback) =>{
    ...
    for(let i=0; i< ids.length; i++){
        for(let j=0; j < db.bu.length; j++){
            if(ids[i] == db.bu[j].id){ buttons.push(db.bu[j].data); break;} // assume id is unique in all kind of collections
        }
    }
    callback(buttons);
}
