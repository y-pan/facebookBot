...
module.exports.cleanStr = (str) =>{
    ...
    return str.replace(/\s{3,}/g," ").replace(/\s{2,}/g," ").trim();
}
module.exports.cleanStrLowerCase = (str) =>{
    return self.cleanStr(str).toLowerCase();
}
module.exports.cleanAndSplit = (str) =>{
    let x = str.replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g," ");  // replace punctuations as space
    x = self.cleanStr(x);
    return x.split(" ");
}
module.exports.getOverlapCountOfArrays = (array1, array2) =>{
    ...
    for(let i=0; i<arri.length; i++){
        for(let j=0; j<arrj.length; j++){
            if(self.cleanStrLowerCase(arri[i]) == self.cleanStrLowerCase(arrj[j])) { 
                count++;
                arri.splice(i,1); i--;
                arrj.splice(j,1); j--;
            }
