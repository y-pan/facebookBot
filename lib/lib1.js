...
module.exports.getTops = (data) => {
    // pass in:  [{"data":camera,"distance":distance}] 
    console.log("@@@getTops beings with size: ..................  " + data.length)
    data.forEach(element => {
        console.log("@d_e="+ element.data.tag + " | " + element.distance)
    });
    let tops = [];
    let sortedData = data.sort(function(a, b){return a.distance - b.distance});

    data.forEach(element => {
        console.log("@d2_e="+ element.data.tag + " | " + element.distance)
    });

    sortedData.forEach(element => {
        console.log("@s_e="+ element.data.tag + " | " + element.distance)
    });
    for(let i=0; i<sortedData.length; i++){
        if(i < vars.result_limit){
            tops.push(sortedData[i]);
        }
    }
    console.log("@@@getTops ends with size: ..................  " + tops.length)
