...
module.exports.getByHighestTagCount = (data, softLimit) => {
    let arr = [];
    data = data.sort((a,b) =>{return b.tagCount - a.tagCount}); // already sort data in place, no need to use 
    console.log("[!!!!!!!!!!!!!!!] after sort by tagCount desc: ")
    data.forEach(d => {
...
module.exports.getByLowestDistance = (data, hardLimit) => {
    ...
    data = data.sort((a,b) => {return a.distance - b.distance}); // sort ascending by distance
    for(let i=0, len=data.length; i<len; i++){
